...
import { UpdateArticleQuotationEntryDto } from '../dtos/buying-article-quotation-entry.update.dto';
import { InvoicingCalculationsService } from 'src/common/calculations/services/invoicing.calculations.service';
import { BuyingArticleQuotationEntryRepository } from '../repositories/repository/buying-article-quotation-entry.repository';
import { BuyingArticleQuotationEntryTaxService } from './buying-article-quotation-entry-tax.service';
...
import { ResponseBuyingArticleQuotationEntryDto } from '../dtos/buying-article-quotation-entry.response.dto';
import { BuyingArticleQuotationEntryEntity } from '../repositories/entities/buying-article-quotation-entry.entity';
import { CreateBuyingArticleQuotationEntryDto } from '../dtos/buying-article-quotation-entry.create.dto';

@Injectable()
export class BuyingArticleQuotationEntryService {
  constructor(
    private readonly articleQuotationEntryRepository: BuyingArticleQuotationEntryRepository,
    private readonly articleQuotationEntryTaxService: BuyingArticleQuotationEntryTaxService,
  ...
  async findOneByCondition(
    query: IQueryObject,
  ): Promise<BuyingArticleQuotationEntryEntity | null> {
    const queryBuilder = new QueryBuilder();
    const queryOptions = queryBuilder.build(query);
    const entry = await this.articleQuotationEntryRepository.findOne(
      queryOptions as FindOneOptions<BuyingArticleQuotationEntryEntity>,
    );
    if (!entry) return null;
    return entry;
  }

  async findOneById(id: number): Promise<ResponseBuyingArticleQuotationEntryDto> {
  ...
  async save(
    createArticleQuotationEntryDto: CreateBuyingArticleQuotationEntryDto,
  ): Promise<BuyingArticleQuotationEntryEntity> {
  ...
  async saveMany(
    createArticleQuotationEntryDtos: CreateBuyingArticleQuotationEntryDto[],
  ): Promise<BuyingArticleQuotationEntryEntity[]> {
    const savedEntries = [];
  ...
  async softDelete(id: number): Promise<BuyingArticleQuotationEntryEntity> {
  ...
  async softDeleteMany(ids: number[]): Promise<BuyingArticleQuotationEntryEntity[]> {
