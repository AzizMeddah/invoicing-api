...
import { SellingQuotationUploadRepository } from '../repositories/repository/selling-quotation-upload.repository';
import { SellingQuotationUploadEntity } from '../repositories/entities/selling-quotation-file.entity';

@Injectable()
export class SellingQuotationUploadService {
  constructor(
    private readonly quotationUploadRepository: SellingQuotationUploadRepository,
    private readonly storageService: StorageService,
  ) {}

  async findOneById(id: number): Promise<SellingQuotationUploadEntity> {
  ...
  async findOneByCondition(
    query: IQueryObject,
  ): Promise<SellingQuotationUploadEntity | null> {
    const queryBuilder = new QueryBuilder();
    const queryOptions = queryBuilder.build(query);
    const upload = await this.quotationUploadRepository.findOne(
      queryOptions as FindOneOptions<SellingQuotationUploadEntity>,
    );
    if (!upload) return null;
    return upload;
  }

  async findAll(query: IQueryObject): Promise<SellingQuotationUploadEntity[]> {
    const queryBuilder = new QueryBuilder();
    const queryOptions = queryBuilder.build(query);
    return await this.quotationUploadRepository.findAll(
      queryOptions as FindManyOptions<SellingQuotationUploadEntity>,
    );
  }

  async findAllPaginated(
    query: IQueryObject,
  ): Promise<PageDto<SellingQuotationUploadEntity>> {
    const queryBuilder = new QueryBuilder();
    const queryOptions = queryBuilder.build(query);
  ...
  async save(
    quotationId: number,
    uploadId: number,
  ): Promise<SellingQuotationUploadEntity> {
  ...
  async softDelete(id: number): Promise<SellingQuotationUploadEntity> {
  ...
  async softDeleteMany(
    quotationUploadEntities: SellingQuotationUploadEntity[],
  ): Promise<SellingQuotationUploadEntity[]> {
