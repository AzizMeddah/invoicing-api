...
import { SellingArticleQuotationEntryTaxService } from './selling-article-quotation-entry-tax.service';
...
import { ResponseSellingArticleQuotationEntryDto } from '../dtos/selling-article-quotation-entry.response.dto';
import { CreateSellingArticleQuotationEntryDto } from '../dtos/selling-article-quotation-entry.create.dto';
import { SellingArticleQuotationEntryRepository } from '../repositories/repository/selling-article-quotation-entry.repository';
import { UpdateSellingArticleQuotationEntryDto } from '../dtos/selling-article-quotation-entry.update.dto';

@Injectable()
export class SellingArticleQuotationEntryService {
  constructor(
    private readonly articleQuotationEntryRepository: SellingArticleQuotationEntryRepository,
    private readonly articleQuotationEntryTaxService: SellingArticleQuotationEntryTaxService,
  ...
  async findOneByCondition(
    query: IQueryObject,
  ): Promise<SellingArticleQuotationEntryEntity | null> {
    const queryBuilder = new QueryBuilder();
    const queryOptions = queryBuilder.build(query);
    const entry = await this.articleQuotationEntryRepository.findOne(
      queryOptions as FindOneOptions<SellingArticleQuotationEntryEntity>,
    );
    if (!entry) return null;
    return entry;
  }

  async findOneById(id: number): Promise<ResponseSellingArticleQuotationEntryDto> {
  ...
  async save(
    createArticleQuotationEntryDto: CreateSellingArticleQuotationEntryDto,
  ): Promise<SellingArticleQuotationEntryEntity> {
  ...
  async saveMany(
    createArticleQuotationEntryDtos: CreateSellingArticleQuotationEntryDto[],
  ): Promise<SellingArticleQuotationEntryEntity[]> {
    const savedEntries = [];
  ...
  async update(
    id: number,
    updateArticleQuotationEntryDto: UpdateSellingArticleQuotationEntryDto,
  ): Promise<SellingArticleQuotationEntryEntity> {
  ...
  async softDelete(id: number): Promise<SellingArticleQuotationEntryEntity> {
  ...
  async softDeleteMany(ids: number[]): Promise<SellingArticleQuotationEntryEntity[]> {
