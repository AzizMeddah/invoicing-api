...
@Permissions({any:[
  ...
  @UseGuards( PermissionsGuard)
@Permissions({all:[PERMISSIONS.ACTIVITY_READ]})
  @Get('/list')
...
@Permissions({all:[PERMISSIONS.ACTIVITY_READ]})
  ...
  @UseGuards( PermissionsGuard)
@Permissions({all:[PERMISSIONS.ACTIVITY_CREATE,PERMISSIONS.ACTIVITY_READ]})
  @Post('')
  ...
  async save(
    ...
    const activity = await this.activityService.save(createActivityDto);
    const performedBy = await this.userService.findOneById(req.user.id);
    req.logInfo = { id: activity.id ,label:activity.label , performedBy:performedBy.username};
    return activity;
  }
  ...
  @UseGuards( PermissionsGuard)
@Permissions({all:[PERMISSIONS.ACTIVITY_UPDATE,PERMISSIONS.ACTIVITY_READ]})
  @Put('/:id')
  ...
  @UseGuards( PermissionsGuard)
@Permissions({all:[PERMISSIONS.ACTIVITY_DELETE,PERMISSIONS.ACTIVITY_READ]})
  @Delete('/:id')
