...
export class RoleService {
  ...
  async save(createRoleDto: CreateRoleDto): Promise<RoleEntity> {
    const existingRole= await this.findOneByCondition({
      filter:`label||$eq||${createRoleDto.label}`
    })
    if(existingRole){
      throw new BadRequestException('roles.errors.role_already_exist')
    }
  ...
  async update(id: number, updateRoleDto: UpdateRoleDto): Promise<RoleEntity> {
        let existingRole= await this.findOneByCondition({
      filter:`label||$eq||${updateRoleDto.label};id||!$eq||${id}`
    })
    if(existingRole){
      throw new BadRequestException('roles.errors.role_already_exist')
    }
    const { permissionsIds, ...sanitizedUpdateRoleDto } = updateRoleDto;
     existingRole = await this.findOneByCondition({
      filter: `id||$eq||${id}`,
      join: 'permissionsEntries',
    });
