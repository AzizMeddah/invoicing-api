...
import { BuyingArticleInvoiceEntryTaxService } from './buying-article-invoice-entry-tax.service';

import { ArticleInvoiceEntryNotFoundException } from '../../errors/article-invoice-entry.notfound.error';
import { BuyingArticleInvoiceEntryRepository } from '../repositories/repository/buying-article-invoice-entry.repository';
import { ResponseBuyingArticleInvoiceEntryDto } from '../dtos/buying-article-invoice-entry.response.dto';
import { BuyingArticleInvoiceEntryEntity } from '../repositories/entities/buying-article-invoice-entry.entity';
import { CreateBuyingArticleInvoiceEntryDto } from '../dtos/buying-article-invoice-entry.create.dto';
import { UpdateBuyingArticleInvoiceEntryDto } from '../dtos/buying-article-invoice-entry.update.dto';


@Injectable()
export class BuyingArticleInvoiceEntryService {
  constructor(
    private readonly articleInvoiceEntryRepository: BuyingArticleInvoiceEntryRepository,
    private readonly articleInvoiceEntryTaxService: BuyingArticleInvoiceEntryTaxService,
  ...
  async findOneByCondition(
    query: IQueryObject,
  ): Promise<BuyingArticleInvoiceEntryEntity | null> {
    const queryBuilder = new QueryBuilder();
    const queryOptions = queryBuilder.build(query);
    const entry = await this.articleInvoiceEntryRepository.findOne(
      queryOptions as FindOneOptions<BuyingArticleInvoiceEntryEntity>,
    );
    if (!entry) return null;
    return entry;
  }

  async findOneById(id: number): Promise<ResponseBuyingArticleInvoiceEntryDto> {
  ...
  async save(
    createArticleInvoiceEntryDto: CreateBuyingArticleInvoiceEntryDto,
  ): Promise<BuyingArticleInvoiceEntryEntity> {
  ...
  async saveMany(
    createArticleInvoiceEntryDtos: CreateBuyingArticleInvoiceEntryDto[],
  ): Promise<BuyingArticleInvoiceEntryEntity[]> {
    const savedEntries = [];
  ...
  async update(
    id: number,
    updateArticleInvoiceEntryDto: UpdateBuyingArticleInvoiceEntryDto,
  ): Promise<BuyingArticleInvoiceEntryEntity> {
  ...
  async softDelete(id: number): Promise<BuyingArticleInvoiceEntryEntity> {
  ...
  async softDeleteMany(ids: number[]): Promise<BuyingArticleInvoiceEntryEntity[]> {
