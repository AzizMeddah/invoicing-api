import { Column, Entity, JoinColumn, ManyToOne, OneToMany, OneToOne, PrimaryGeneratedColumn } from 'typeorm';
import { BuyingQuotationEntity } from 'src/modules/quotation/buying-quotation/repositories/entities/buying-quotation.entity';
import { BuyingPaymentInvoiceEntryEntity } from 'src/modules/payment/buying-payment/repositories/entities/buying-payment-invoice-entry.entity';
import { EntityHelper } from 'src/common/database/interfaces/database.entity.interface';
import { BUYING_INVOICE_STATUS } from '../../enums/buying-invoice-status.enum';
import { DISCOUNT_TYPES } from 'src/app/enums/discount-types.enum';
import { CurrencyEntity } from 'src/modules/currency/repositories/entities/currency.entity';
import { FirmEntity } from 'src/modules/firm/repositories/entities/firm.entity';
import { InterlocutorEntity } from 'src/modules/interlocutor/repositories/entity/interlocutor.entity';
import { CabinetEntity } from 'src/modules/cabinet/repositories/entities/cabinet.entity';
import { BuyingArticleInvoiceEntryEntity } from './buying-article-invoice-entry.entity';
import { BuyingInvoiceMetaDataEntity } from './buying-invoice-meta-data.entity';
import { BankAccountEntity } from 'src/modules/bank-account/repositories/entities/bank-account.entity';
import { BuyingInvoiceUploadEntity } from './buying-invoice-file.entity';
import { TaxEntity } from 'src/modules/tax/repositories/entities/tax.entity';
import { TaxWithholdingEntity } from 'src/modules/tax-withholding/repositories/entities/tax-withholding.entity';
import { UploadEntity } from 'src/common/storage/repositories/entities/upload.entity';
import { FirmBankAccountEntity } from 'src/modules/firm-bank-account/repositories/entities/firm-bank-account.entity';

@Entity('buying-invoice')
export  class BuyingInvoiceEntity extends EntityHelper{

    @PrimaryGeneratedColumn()
    id: number;

    @Column({ type: 'varchar', length: 25,nullable:true})
    sequential: string;

    @Column({ nullable: true })
    date: Date;

    @Column({ nullable: true })
    dueDate: Date;

    @Column({ type: 'varchar', length: 255, nullable: true })
    object: string;

    @Column({ type: 'varchar', length: 1024, nullable: true })
    generalConditions: string;

    @Column({ type: 'enum', enum: BUYING_INVOICE_STATUS, nullable: true })
    status: BUYING_INVOICE_STATUS;

    @Column({ nullable: true })
    discount: number;

    @Column({ type: 'enum', enum: DISCOUNT_TYPES, nullable: true })
    discount_type: DISCOUNT_TYPES;

    @Column({ type: 'float', nullable: true })
    subTotal: number;

    @Column({ type: 'float', nullable: true })
    total: number;

    @Column({ type: 'float', nullable: true })
    amountPaid: number;

    @ManyToOne(() => CurrencyEntity)
    @JoinColumn({ name: 'currencyId' })
    currency: CurrencyEntity;

    @Column({ type: 'int' })
    currencyId: number;

    @ManyToOne(() => FirmEntity)
    @JoinColumn({ name: 'firmId' })
    firm: FirmEntity;

    @Column({ type: 'int' })
    firmId: number;

    @ManyToOne(() => InterlocutorEntity)
    @JoinColumn({ name: 'interlocutorId' })
    interlocutor: InterlocutorEntity;

    @ManyToOne(() => CabinetEntity)
    @JoinColumn({ name: 'cabinetId' })
    cabinet: CabinetEntity;

    @Column({ type: 'int', default: 1 })
    cabinetId: number;

    @Column({ type: 'int' })
    interlocutorId: number;

    @Column({ type: 'varchar', length: 1024, nullable: true })
    notes: string;

    @OneToMany(() => BuyingArticleInvoiceEntryEntity, (entry) => entry.invoice)
    articleInvoiceEntries: BuyingArticleInvoiceEntryEntity[];

    @OneToOne(() => BuyingInvoiceMetaDataEntity)
    @JoinColumn()
    invoiceMetaData: BuyingInvoiceMetaDataEntity;

    @ManyToOne(() => FirmBankAccountEntity)
    @JoinColumn({ name: 'bankAccountId' })
    bankAccount: FirmBankAccountEntity;

    @Column({ type: 'int' })
    bankAccountId: number;

    @OneToMany(() => BuyingInvoiceUploadEntity, (upload) => upload.invoice)
    uploads: BuyingInvoiceUploadEntity[];

    @ManyToOne(() => TaxEntity)
    @JoinColumn({ name: 'taxStampId' })
    taxStamp: TaxEntity;

    @Column({ type: 'int' })
    taxStampId: number;

    @ManyToOne(() => TaxWithholdingEntity)
    @JoinColumn({ name: 'taxWithholdingId' })
    taxWithholding: TaxWithholdingEntity;

    @Column({ type: 'int', nullable: true })
    taxWithholdingId: number;

    @Column({ type: 'float', nullable: true })
    taxWithholdingAmount: number;

    @ManyToOne(() => BuyingQuotationEntity)
    @JoinColumn({ name: 'quotationId' })
    quotation: BuyingQuotationEntity;

    @Column({ type: 'int', nullable: true })
    quotationId: number;

    @OneToMany(() => BuyingPaymentInvoiceEntryEntity, (entry) => entry.invoice)
    payments: BuyingPaymentInvoiceEntryEntity[];

    @OneToOne(() => UploadEntity)
    @JoinColumn({ name: 'referenceDocId' })
    referenceDoc: UploadEntity;

    @Column({ type: 'int' })
    referenceDocId: number;
}
