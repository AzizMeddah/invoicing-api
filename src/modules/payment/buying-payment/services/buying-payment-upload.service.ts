...
import { BuyingPaymentUploadRepository } from '../repositories/repository/buying-payment-file.repository';
import { BuyingPaymentUploadEntity } from '../repositories/entities/buying-payment-file.entity';

@Injectable()
export class BuyingPaymentUploadService {
  constructor(
    private readonly paymentUploadRepository: BuyingPaymentUploadRepository,
    private readonly storageService: StorageService,
  ) {}

  async findOneById(id: number): Promise<BuyingPaymentUploadEntity> {
  ...
  async findOneByCondition(
    query: IQueryObject,
  ): Promise<BuyingPaymentUploadEntity | null> {
    const queryBuilder = new QueryBuilder();
    const queryOptions = queryBuilder.build(query);
    const upload = await this.paymentUploadRepository.findOne(
      queryOptions as FindOneOptions<BuyingPaymentUploadEntity>,
    );
    if (!upload) return null;
    return upload;
  }

  async findAll(query: IQueryObject): Promise<BuyingPaymentUploadEntity[]> {
    const queryBuilder = new QueryBuilder();
    const queryOptions = queryBuilder.build(query);
    return await this.paymentUploadRepository.findAll(
      queryOptions as FindManyOptions<BuyingPaymentUploadEntity>,
    );
  }

  async findAllPaginated(
    query: IQueryObject,
  ): Promise<PageDto<BuyingPaymentUploadEntity>> {
    const queryBuilder = new QueryBuilder();
    const queryOptions = queryBuilder.build(query);
  ...
  async save(
    paymentId: number,
    uploadId: number,
  ): Promise<BuyingPaymentUploadEntity> {
    return this.paymentUploadRepository.save({ paymentId, uploadId });
  }

  async duplicate(id: number, paymentId: number): Promise<BuyingPaymentUploadEntity> {
  ...
  async softDelete(id: number): Promise<BuyingPaymentUploadEntity> {
  ...
  async softDeleteMany(
    quotationUploadEntities: BuyingPaymentUploadEntity[],
  ): Promise<BuyingPaymentUploadEntity[]> {
